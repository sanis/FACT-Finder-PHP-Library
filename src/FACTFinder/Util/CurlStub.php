...
class CurlStub implements CurlInterface
{
    ...
    private $mapOptionCounts = [];
    private $mapOptions = [];
    private $mapResponses = [];
    private $mapErrorCodes = [];
    private $mapInfo = [];
    /**
     * @var CurlHandle[]
     */
    ...
    public function multi_add_handle($mh, $ch)
    {
        ...
        $this->handles[$ch]->durationLeft = mt_rand(0, 5);
     ...
     */
    public function setResponse($expectedResponse, $requiredOptions = [])
    {
        $hash = $this->getHashAndSetOptionMaps($requiredOptions);
        $this->mapResponses[$hash] = $expectedResponse;
    }

    public function setErrorCode($expectedErrorCode, $requiredOptions = [])
    {
        $hash = $this->getHashAndSetOptionMaps($requiredOptions);
        $this->mapErrorCodes[$hash] = $expectedErrorCode;
    }

    public function setInformation($expectedInfo, $requiredOptions = [])
    {
        $hash = $this->getHashAndSetOptionMaps($requiredOptions);
